...
function OrderListPage() {
  ...
  const loadCart = async () => {
    try {
      ...
      setError(error.message || "No se pudo cargar la lista.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSendWhatsApp = async () => {
    try {
      ...
      const data = await generateCartWhatsApp(cliente);
    ...
    <section className="container-smika py-12">
      <div className="rounded-[32px] bg-[#F8F6F7] p-8 mb-8">
        ...
        <p className="mt-3 text-gray-600">
          Aquí aparecerán los productos que el cliente agregue antes de enviar
