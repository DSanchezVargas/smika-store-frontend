...
async function parseResponse(response) {
  ...
  if (!response.ok) {
    throw new Error(data.message || "Ocurrió un error con países/orígenes");
  }

  return data;
}

export async function getOrigins(params = {}) {
  const query = new URLSearchParams(params).toString();
  const url = query ? `${API_URL}/origins?${query}` : `${API_URL}/origins`;
